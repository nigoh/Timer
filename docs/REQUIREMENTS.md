...
- 停止操作は「実行中」または「一時停止中」のみ活性（待機中は無効）
- セッションラベル編集
- 履歴表示・削除・全削除
...
- アジェンダ表示とタイマー連携UI
- 作業単位の進行支援
- 会議作成（＋新しい会議）と会議名編集をダイアログ操作で提供
- 停止系操作の定義: 「一時停止」は経過時間を保持して再開可能、「セッション完了」は現在議題を完了として次の議題へ遷移する
- 完了ボタン（セッション完了）は「実行中」または「一時停止中」または「超過中」のみ活性（待機中は無効）
- 厳密運用モード: 「次へ」は実行中に無効化し、議題移動は一時停止/セッション完了後のみ実行可能（「前へ」は提供しない）
- 「次へ」は `running` / `paused` / `overtime` の議題のみ実行可能（未開始 `pending` からのスキップ完了は禁止）
- 各議題に議事録入力欄を持ち、`Rich Text` と `Markdown` の入力形式を切り替え可能
- 各議題にセクション状態（未開始 / 進行中 / 終了 / 保留）を表示・更新可能
- アジェンダ一覧は右サイドバーに表示し、開閉トグルとモバイルドロワー表示を提供
...
- 各ログを生成AI向け分析文として個別にクリップボードへコピー
...
- 表示中の各ログを生成AI分析用テンプレートに整形して個別コピー可能
