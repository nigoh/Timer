...
import { useAgendaTimerStore } from '../stores/new-agenda-timer-store';
...
export const Dashboard: React.FC = () => {
  const { filter, setGranularity, setDateRange, setTimerKind } = useDashboardStore();
  ...
  const rawData: RawData = useMemo(
    ...
    [basicHistory, pomodoroSessions, meetings, multiCompleted, multiTimers.length, multiCategoryMap],
  );
  ...
  const handleExportCsv = () => {
    const rows = [
      ...
      ...result.trend.map((p) => [p.label, p.focusMinutes, p.sessions, p.completedSessions]),
    ];
