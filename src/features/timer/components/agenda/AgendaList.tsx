...
import { getSuggestionsForAgenda } from "@/features/timer/services/meeting-optimization-service";
...
export const AgendaList: React.FC<AgendaListProps> = ({
  ...
  const recordCount = useMeetingKnowledgeStore((s) => s.records.length);
  const knowledgeEnabled = useMeetingKnowledgeStore((s) => s.settings.enabled);

  const currentAgenda = getCurrentAgenda();

  const suggestions = useMemo(() => {
    if (!currentMeeting || !knowledgeEnabled) return [];
    return getSuggestionsForAgenda(currentMeeting.agenda);
  }, [currentMeeting, knowledgeEnabled, recordCount]); // eslint-disable-line react-hooks/exhaustive-deps
    ...
    <>
      ...
      {activeSuggestion && (
        <SuggestionDialog
          ...
          agendaTitle={
            currentMeeting.agenda.find(
              (a) => a.id === activeSuggestion.agendaId,
            )?.title ?? ""
          }
          ...
          onApply={() => {
            updateAgenda(currentMeeting.id, activeSuggestion.agendaId, {
              plannedDuration: activeSuggestion.suggestedValue,
            });
