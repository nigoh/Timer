...
describe('VoiceTranscriptPanel', () => {
  ...
  const renderPanel = async (props: Partial<React.ComponentProps<typeof VoiceTranscriptPanel>> = {}) => {
    const defaultProps = {
      meetingId: 'meeting-1',
      agendaId: 'agenda-1',
  ...
  it('ヘッダークリックでパネルが開く', async () => {
    ...
    expect(container.textContent).toContain('録音を開始すると文字起こしが表示されます');
  });
  ...
  it('minutesFormat=markdown のとき「議事録に追加」ボタンが表示される', async () => {
    mockVoiceHook.isListening = true;
    await renderPanel({ minutesFormat: 'markdown' });

    const buttons = Array.from(container.querySelectorAll('button'));
    const insertBtn = buttons.find(b => b.textContent?.includes('議事録に追加'));
    expect(insertBtn).toBeTruthy();
  });

  it('minutesFormat=richtext のとき「AI要約して議事録に追加」ボタンが表示される', async () => {
    mockVoiceHook.isListening = true;
    await renderPanel({ minutesFormat: 'richtext' });

    const buttons = Array.from(container.querySelectorAll('button'));
    const insertBtn = buttons.find(b => b.textContent?.includes('AI要約'));
    expect(insertBtn).toBeTruthy();
  });
  ...
  it('markdown 形式で「議事録に追加」クリックすると updateAgendaMinutes が呼ばれる', async () => {
    ...
    const buttons = Array.from(container.querySelectorAll('button'));
    const insertBtn = buttons.find(b => b.textContent?.includes('議事録に追加') && !b.disabled)!;
  ...
  it('richtext 形式で「AI要約して議事録に追加」クリックすると onRequestSummaryDialog が呼ばれる', async () => {
    ...
    const buttons = Array.from(container.querySelectorAll('button'));
    const insertBtn = buttons.find(b => b.textContent?.includes('AI要約') && !b.disabled)!;
  ...
  it('クリアボタンをクリックすると clearTranscript が呼ばれる', async () => {
    ...
    const buttons = Array.from(container.querySelectorAll('button'));
    const clearBtn = buttons.find(b => b.getAttribute('aria-label') === '文字起こしをクリア');
