...
### タスク 1.1: `PomodoroInstanceState` に `lastTickTime` を追加する

- [x] `src/features/timer/stores/pomodoro-store.ts` の `PomodoroInstanceState` に `lastTickTime: number | null` を追加する
...
- [x] `set()` 内で完了時は `lastTickTime: null`、継続時は `lastTickTime: now` を更新する
